...
import { saveSettings, autoConfigureProxy } from '../services/settingsService';
...
router.post('/auto-proxy', async (req, res) => {
  const logs: string[] = [];
  try {
    const proxyUrl = await autoConfigureProxy((msg) => logs.push(msg));
    if (proxyUrl) {
      res.json({ success: true, logs, proxyUrl, message: `Working proxy found and saved: ${proxyUrl}` });
    } else {
      res.json({ success: false, logs, message: 'No working proxy found. Try again in a few minutes.' });
    }
  } catch (error: any) {
